...
class CClient(object):
    def __init__(self, server, port, working_dir, lib_path):
        ...
        self.obj = lib.create_instance(ctype.c_char_p(str(server).encode('utf-8')), port)
    def get_file(self, filename, newfilename, delimit=' ', quotech='|'):
        if not os.path.exists(self.working_dir + newfilename):
    ...
    def get_info(self, name, date, delimit=' ', quotech='|'):
        ...
        return get_file(filename, filename, delimit, quotech)
